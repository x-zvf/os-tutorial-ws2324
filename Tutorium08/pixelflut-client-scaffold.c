...
void flood(int fd, uint32_t const *argbbuf, int w, int h, int x, int y) {
    ...
    //      The (w x h) image should appear at (x, y) on the server
    //      argbbuf contains the loaded image in ARGB format (layeed row-by-row)
    //      For example pixel (1, 0) is at argbbuf[width]. If it were green, 
    //      it would be 0x__ff0000,
